...
class MasEntriesWizard(models.TransientModel):
    ...
    def _open_next_period_flag_enable(self):
        for record in self:
            if record.period_id:
                if record.period_id.end_date <= date.today():
    ...
    def moves_compute(self):
        ...
        asset_move_ids = self.env['asset_management.asset'].generate_mas_entries(self.period_id, self.post_entries,
                                                                                 self.book_id.id, run_number_process.id)
